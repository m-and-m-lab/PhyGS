...
def sample_home_constraint_params(
    ...
    # Each fullness param defaults to its original random distribution when left
    # as None. Override from gin with either a scalar (e.g. 0.9) or a
    # random_general spec (e.g. ("uniform", 0.8, 0.95)) to pin object density.
    return dict(
        # what pct of the room floorplan should we try to fill with furniture?
        furniture_fullness_pct=(
            uniform(0.6, 0.9)
            if furniture_fullness_pct is None
            else rg(furniture_fullness_pct)
        ),
        # how many objects in each shelving per unit of volume
        obj_interior_obj_pct=(
            uniform(0.5, 1)
            if obj_interior_obj_pct is None
            else rg(obj_interior_obj_pct)
        ),  # uniform(0.6, 0.9),
        # what pct of top surface of storage furniture should be filled with objects? e.g pct of top surface of shelf
        obj_on_storage_pct=(
            uniform(0.5, 1.0)
            if obj_on_storage_pct is None
            else rg(obj_on_storage_pct)
        ),
        ...
        obj_on_nonstorage_pct=(
            uniform(0.2, 1.0)
            if obj_on_nonstorage_pct is None
            else rg(obj_on_nonstorage_pct)
        ),
...
@gin.configurable
def home_room_constraints(has_fewer_rooms=False, n_bedrooms=None):
    ...
    if n_bedrooms is not None:
        # Force an exact total bedroom count (hard constraint). Applies to both
        # the standard and has_fewer_rooms node_constraint branches above.
        node_constraint = node_constraint * (
            rooms[Semantics.Bedroom].count() == n_bedrooms
        )
