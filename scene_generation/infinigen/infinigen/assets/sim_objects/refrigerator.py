...
class RefrigeratorFactory(AssetFactory):
    ...
    def create_asset(self, asset_params=None, **kwargs):
        # When called from articulation_tagging.export_articulated_twin via
        # spawn_simready, return the in-scene copy directly so the articulated
        # USD twin matches the in-scene geometry. See SimDoorFactory.create_asset.
        if "existing_asset" in kwargs:
            return kwargs["existing_asset"]
