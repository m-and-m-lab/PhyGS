...
class OvenFactory(AssetFactory):
    ...
    def create_asset(self, **params):
        ...
        if not self.params["UseGas"]:
            # Electric-oven early return — solver places this same `obj`, so
            # tag here in addition to the gas-oven return at the bottom.
            from infinigen.core.sim.articulation_tagging import (
                export_articulated_twin,
            )
            export_articulated_twin(obj, "oven")
            return obj
        width, depth = (
            self.params["Width"],
        ...
        from infinigen.core.sim.articulation_tagging import export_articulated_twin
        export_articulated_twin(obj, "oven")
