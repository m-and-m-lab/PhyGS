...
class SingleCabinetBaseFactory(AssetFactory):
    ...
    def create_asset(self, i=0, **params):
        ...
        # Articulated USD twin: mirror this cabinet's source parts into a standalone
        # articulated USD via SimCabinetFactory. Done BEFORE the SingleCabinet's own
        # geomod (line below) consumes shelf into obj; SimCabinet needs the loose
        # parts to construct the articulated chain. The in-scene obj continues
        # through the rest of create_asset to produce the (door-less) cabinet body
        # placed by the constraint solver; the loose right_door / left_door are
        # deleted at the end of this method since their geometry is captured in the
        # articulated twin and otherwise they'd appear as floating static prims.
        ...
        copyobj = obj.copy(); copyobj.data = obj.data.copy()
        ...
        asset_usd_path = (
            _Path("./sim_exports/usd/cabinet") / str(articulated_name) / "cabinet.usda"
        ).resolve()
        ...
        if len(cabinet_params["door_hinge_pos"]) > 1:
            ...
            loose_doors.append(left_door)
        else:
            butil.delete([left_door])

        tagging.tag_system.relabel_obj(obj)

        # The loose right_door / left_door objects are not parented to obj and would
        # otherwise survive into the baked scene USD as floating static prims at
        # whatever world coords door_hinge_pos resolves to (which is not the
        # constraint-solver-chosen cabinet pose). Their geometry is already inside
        # the articulated USD twin we exported above, so delete them here.
        butil.delete(loose_doors)
