...
class CabinetBaseFactory(AssetFactory):
    ...
    def create_asset(self, i=0, **params):
        ...
        # Articulated USD twin: mirror this cabinet (joined obj + its source parts)
        # into a standalone articulated USD via SimCabinetFactory, then tag the in-scene
        # obj so tools/export.py:extract_articulated_assets can record its world pose
        # and remove its baked geometry. The composer then injects a USD reference at
        # the recorded pose. See doors/base.py for the equivalent pattern on doors.
        #
        # CRITICAL: only export on the in-scene-creation branch. SimCabinetFactory
        # inherits from CabinetBaseFactory and does NOT override create_asset, so when
        # spawn_simready calls SimCabinetFactory.spawn_asset, control re-enters this
        # method with existing_asset+copy_from_existing set. If we exported again on
        # that branch we'd recurse forever. The condition mirrors line 1914.
        #
        # Snapshot bpy.data.objects BEFORE making any copies so the snapshot-diff
        # cleanup catches the four part-copies and any sim wrappers spawn_simready
        # leaves behind (renamed copyobj, plus any SimCabinet helper objects).
        is_inscene_creation = (
            "existing_asset" not in params or not params.get("copy_from_existing")
        )
        articulated_name = None
        if is_inscene_creation:
            from pathlib import Path as _Path

            from scripts.usd_articulated_asset_exporter import (
                export_articulated_asset,
            )
            _objs_before = {o.as_pointer() for o in bpy.data.objects}
            copycabinet = obj.copy(); copycabinet.data = obj.data.copy()
            copyshelf = shelf.copy(); copyshelf.data = shelf.data.copy()
            copyright = right_door.copy(); copyright.data = right_door.data.copy()
            copyleft = left_door.copy(); copyleft.data = left_door.data.copy()
            articulated_name = export_articulated_asset(
                copycabinet, "cabinet",
                cabinet_params={
                    "shelf": copyshelf,
                    "right_door": copyright,
                    "left_door": copyleft,
            ...
            for _new in [
                o for o in bpy.data.objects if o.as_pointer() not in _objs_before
            ]:
        ...
        if is_inscene_creation:
            asset_usd_path = (
                _Path("./sim_exports/usd/cabinet")
                / str(articulated_name)
                / "cabinet.usda"
            ).resolve()
            obj["sim_articulated_kind"] = "cabinet"
            obj["sim_articulated_idx"] = int(articulated_name)
            obj["sim_articulated_usd_path"] = str(asset_usd_path)
