...
def build_scene_asset(args, factory_name, idx):
    ...
    if hasattr(fac, 'get_all_params'):
        params = fac.get_all_params()
    else:
        params = {}
...
def build_and_save_asset(payload: dict):
    ...
    if "cat" in payload:
        output_folder = args.output_folder / payload["cat"] #/ f"{factory_name}_{idx:03d}"
    else:
        output_folder = args.output_folder
    file_name = f'{factory_name}_{idx:03d}'
    ...
    gin.add_config_file_search_path('third_party/infinigen')
    ...
    if "Factory" in factory_name:
        asset, params = build_scene_asset(args, factory_name, idx)
    else:
    ...
    with (output_folder / f"{file_name}.txt").open("w") as f:
        save_polycounts(f)
    ...
    if args.save_blend:
        butil.save_blend(output_folder / f"{file_name}.blend", autopack=True)
        # tag_system.save_tag(output_folder / f"{file_name}.json")
        # input(params)
        with open(output_folder / f"{file_name}.json", "w") as f:
...
def main(args):
    ...
    if len(factories) == 1 and factories[0].endswith(".txt"):
        categories = [
            f.split(".")[-2] for f in load_txt_list(factories[0], skip_sharp=False)
        ]
        factories = [
    ...
    if categories is None:
        ...
        targets = [
            {"args": args, "cat": cat, "fac": fac, "idx": idx}
            for idx in range(args.n_images)
            for cat, fac in zip(categories, factories)
        ]
